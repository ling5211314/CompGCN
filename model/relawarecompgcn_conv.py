...
class RelAwareCompGCNConv(MessagePassing):
    def __init__(self, in_channels, out_channels, num_rels, act=lambda x: x, params=None):
        ...
        self.att_param = get_param((in_channels * 2, 1))  # Attention parameter
    ...
    def message(self, x_j, edge_type, rel_embed, edge_norm, mode):
        ...
        h = torch.cat([x_j, rel_emb], dim=-1)
        att_score = torch.matmul(h, self.att_param)  # shape: [E, 1]
        att_score = torch.sigmoid(att_score)
        out = out * att_score  # weighted by attention
